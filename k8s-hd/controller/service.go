...
func(s *servicev1) GetServiceDetail(ctx *gin.Context) {
	...
	data, err := service.Servicev1.GetServicetDetail(client, params.ServiceName, params.Namespace)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{
			"msg": err.Error(),
			"data": nil,
...
func(s *servicev1) CreateService(ctx *gin.Context) {
	...
	if err = service.Servicev1.CreateService(client, serviceCreate); err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{
			"msg": err.Error(),
			"data": nil,
		})
...
func(s *servicev1) UpdateService(ctx *gin.Context) {
	...
	err = service.Servicev1.UpdateService(client, params.Namespace, params.Content)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{
			"msg": err.Error(),
			"data": nil,
